...
// headers) and exposes only this thin launcher to the host glue in fpi.cc.
//
...
//
// Restrictions for the first cut: every modality's A_dependencies rank must
// be in [1, 3]. Modalities with K >= 4 fall back to the FpiCudaHost shim or
...
// Per-modality dispatch metadata. Mirrors fpi.cc's ModalityDispatch but
