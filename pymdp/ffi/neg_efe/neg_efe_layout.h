...
inline FfiError validate_layout(const Layout& L) {
  ...
  for (int64_t f = 0; f < L.F; ++f) {
    ...
    const DependencyView deps = factor_transition_deps(L, f);
    PYMDP_TRY(validate_dep_rank_and_factors(kEfeKernelName, "B_dep", L.F, deps.rank, deps.factors));
    int64_t K = 1;
    for (int i = 0; i < deps.rank; ++i) {
      K *= L.S[deps.factors[i]];
    }
  ...
  for (int64_t m = 0; m < L.M; ++m) {
    ...
    PYMDP_TRY(validate_dep_rank_and_factors(kEfeKernelName, "A_dep", L.F, deps.rank, deps.factors));
    int64_t K = 1;
    for (int i = 0; i < deps.rank; ++i) {
      K *= L.S[deps.factors[i]];
    }
