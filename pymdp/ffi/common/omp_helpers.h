...
#include "common/cpu_capabilities.h"
...
// Conditional `omp parallel for` over `[0, work_units)`. Captures the explicit
// serial/parallel split both kernels use to dodge libomp's 1-thread-team
// overhead: an `omp parallel if(false)` region still creates a 1-thread team
// and pays libomp bookkeeping, so we want a real branch around the `#pragma
// omp parallel` directive, not a runtime predicate inside one.
//
// - `setup()` runs once per thread in the parallel path and once at the top
//   level in the serial path; its return value is forwarded to each `body(b,
//   state)` call. Use it to acquire per-thread scratch.
// - `body(b, state)` runs each work unit. Pragmas inside the body (e.g.
//   `#pragma omp critical` for shared error capture) compose normally.
template <class Setup, class Body>
inline void omp_fire_if(bool fire_parallel, int num_threads, int64_t work_units, Setup&& setup, Body&& body) {
  if (!fire_parallel) {
    auto state = setup();
    for (int64_t b = 0; b < work_units; ++b) body(b, state);
  } else {
