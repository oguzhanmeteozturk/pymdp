...
// Content-tag sampling layout: a 16-element prefix plus 8 strided positions.
constexpr int kContentTagStrideSamples = 8;
constexpr int kContentTagPrefixMax     = 16;

// Index of the slot-th strided sample within a buffer of length `size`.
inline int64_t content_tag_stride_index(int slot, int64_t size) {
  int64_t idx = 0;
  switch (slot) {
  case 0:
    idx = 0;
...
inline uint64_t content_tag(const float* data, int64_t size) {
  ...
  // Raw-bit reads (memcpy to uint32, not a float load): a float load could
  // canonicalize an sNaN bit pattern under -ffast-math, so int-reinterpreted
  // buffers (the policy matrix) would tag inconsistently.
  ...
  for (int i = 0; i < kContentTagStrideSamples; ++i) {
    uint32_t bits;
    std::memcpy(&bits, &data[content_tag_stride_index(i, size)], sizeof(bits));
    h = fnv1a64_mix(h, static_cast<uint64_t>(bits));
  }
  return h;
}
